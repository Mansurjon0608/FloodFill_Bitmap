...
object BitmapGenerator {
    ...
    fun generate(
        ...
        val bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565, false)
    ...
    private fun addRocketLogo(width: Int, height: Int, random: Random, bitmap: Bitmap) {
        ...
        val size = kotlin.random.Random.nextInt(5, max(6, width / 8)).toFloat()

        val rocketY = kotlin.random.Random.nextInt((size / 2).toInt(), max((size / 2).toInt(), height)).toFloat()
        val rocketX = kotlin.random.Random.nextInt((size / 2).toInt(), max((size / 2).toInt(), width)).toFloat()
